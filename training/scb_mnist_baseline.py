...
log = logging.getLogger("SCB-Enhanced-Fixed")
...
@dataclass
class EnhancedSCBConfig:
    """Enhanced configuration with comprehensive experimental parameters"""
    # Core experiment settings
    random_seed: int = 42
    target_class: int = 7
    non_target_class: int = 3
    ...
    poison_ratios: List[float] = field(default_factory=lambda: [0.1, 0.2, 0.4, 0.6])
    epsilon_values: List[float] = field(default_factory=lambda: [0.1, 0.2, 0.3, 0.4])
    admix_params: List[Tuple[float, float]] = field(default_factory=lambda: [(0.5, 1.0), (1.0, 2.0), (1.5, 3.0)])
    n_seeds: int = 5
    
    # Defense parameters
    min_clean_accuracy: float = 85.0
    spectral_threshold: float = 1.5
    fpr_threshold: float = 0.01  # 1% FPR for threshold selection
    ...
    # Output settings
    output_dir: str = "results/enhanced_scb_experiment"
    save_models: bool = True
    
    def __post_init__(self):
        if self.pca_components is None:
            self.pca_components = min(32, 2 ** self.n_qubits // 8)
...
class TriggerGeneratorAlgorithm1:
    ...
    def __init__(self, proxy_model: QuantumNeuralNetwork, config: EnhancedSCBConfig, device: str):
...
class EnhancedDataManager:
    ...
    def __init__(self, config: EnhancedSCBConfig, device: str):
        self.config = config
        self.device = torch.device(device)
        self.device_manager = RobustDeviceManager(device)
...
class QuantumFeatureAnalyzer:
    ...
    def __init__(self, model, config: EnhancedSCBConfig, device: str):
        self.model = model
        self.config = config
        self.device = torch.device(device)
        self.device_manager = RobustDeviceManager(device)
    ...
    def validate_class_separability(self, data_splits: Dict[str, Dict[str, torch.Tensor]],
                                split: str = 'train', n_samples_per_class: int = 200) -> Dict[str, float]:
        """
        Compute clean-sample separability (target vs non-target), consistent with BloodMNIST protocol.
        data_splits: {'train': {class_id: Tensor[N,D]}, ...}
        split: 'train' / 'val' / 'test'
        """
        log.info("🔍 Validating class separability (clean target vs clean non-target)...")
        self.model.eval()
        model_device = self.model.get_device()
        ...
        with torch.no_grad():
            ...
            # non-target class (clean)
            for x in data_splits[split][self.config.non_target_class][:n_samples_per_class]:
                x_dev = self.device_manager.safe_to(x.unsqueeze(0), model_device)
                f = self.model.get_quantum_features(x_dev)
                non_target_feats.append(f.cpu().numpy())

        import numpy as np
        t = np.vstack(target_feats)
        n = np.vstack(non_target_feats)

        t_mean = t.mean(0)
        n_mean = n.mean(0)
        between = float(np.linalg.norm(t_mean - n_mean))
        t_var = float(np.var(t, axis=0).mean())
        n_var = float(np.var(n, axis=0).mean())
        avg_within = (t_var + n_var) / 2.0
        sep = between / (np.sqrt(avg_within) + 1e-12)

        log.info(f"✅ Clean separability: ratio={sep:.4f} (between={between:.4f}, within={avg_within:.4e})")
        ...
        return {
            ...
            "n_non_target_samples": int(n.shape[0])
        }
...
class StatisticalDistanceCalculator:
    ...
    def __init__(self, config: EnhancedSCBConfig):
        self.config = config
        self.statistics = {}
...
class DefenseEvaluator:
    """Comprehensive defense evaluation with proper threshold selection - Fixed ROC direction"""
    
    def __init__(self, config: EnhancedSCBConfig):
        self.config = config
        self.threshold = None
        self.roc_data = {}
        # If validation AUC<0.5 we may flip score sign for ROC direction.
        # This MUST be applied consistently on test when using the selected threshold.
        self.score_flipped = False
    ...
    def select_threshold_on_validation(self, distance_calculator: StatisticalDistanceCalculator,
                                     model, val_clean_data: torch.Tensor,
                                     val_triggered_data: torch.Tensor) -> float:
        """Select optimal threshold using validation data with ROC analysis - FIXED scoring with sanity check"""
        log.info("🎯 Selecting optimal threshold on validation data...")
        
        # Calculate distances on validation data
        ...
        # FIXED: Sanity check - triggered samples should have higher scores on average
        log.info(f"🔍 Sanity check: mean(score_clean)={clean_scores.mean():.3f}, "
                f"mean(score_triggered)={triggered_scores.mean():.3f}  # Expected: triggered > clean")
        
        all_scores = np.concatenate([clean_scores, triggered_scores])
        all_labels = np.concatenate([
            np.zeros(len(clean_scores)),    # 0 = clean (negative class)
            np.ones(len(triggered_scores))  # 1 = triggered (positive class)
        ])
        ...
        # FIXED: Auto-flip scores if direction is wrong (robustness check)
        if roc_auc < 0.5:
            log.warning(f"⚠️ Score direction appears reversed (AUC={roc_auc:.3f} < 0.5), auto-flipping scores")
            all_scores = -all_scores
            fpr, tpr, thresholds = roc_curve(all_labels, all_scores)
            roc_auc = auc(fpr, tpr)
            log.info(f"✅ After flip: AUC={roc_auc:.3f}")
            self.score_flipped = True
        else:
            self.score_flipped = False

        # IMPORTANT: sklearn.metrics.roc_curve may include an initial threshold=inf.
        # If selected, then "score > threshold" is always False -> TPR=FPR=0 -> escape_rate=100%.
        ...
        # Select threshold based on FPR constraint or Youden's J
        if self.config.fpr_threshold is not None:
            target_fpr = self.config.fpr_threshold
            idx = np.argmin(np.abs(fpr - target_fpr))
            selected_threshold = thresholds[idx]
            selected_tpr = tpr[idx]
            selected_fpr = fpr[idx]
            selection_method = f"FPR={target_fpr:.3f}"
        else:
            ...
            selected_fpr = fpr[best_idx]
            selection_method = "Youden's J"
        
        self.threshold = selected_threshold
        
        log.info(f"✅ Threshold selected ({selection_method}): {selected_threshold:.4f}")
        log.info(f"   TPR: {selected_tpr:.3f}, FPR: {selected_fpr:.3f}, AUC: {roc_auc:.3f}")
        log.info(f"   Score flip applied on validation: {self.score_flipped}")
    ...
    def evaluate_defense_on_test(self, distance_calculator: StatisticalDistanceCalculator,
        ...
        # Sync the score direction with validation (critical correctness)
        if self.score_flipped:
...
class ExperimentRunner:
    ...
    def __init__(self, config: EnhancedSCBConfig):
        ...
        self.data_manager = EnhancedDataManager(config, self.device_str)
        self.results = {}
    ...
    def run_single_seed_experiment(self, seed: int) -> Dict[str, Dict]:
        ...
        clean_model, clean_sep_epochs = self._train_clean_baseline(processed_splits)
        ...
        for poison_ratio in self.config.poison_ratios:
            ...
            # Create poisoned dataset and record poisoned-sample indices and tensors for downstream analysis
            train_loader, poison_meta = self._create_poisoned_dataset(
            ...
            # save per-config poisoned sample metadata (indices + tensors) as raw data only
            try:
                # Avoid overwriting across multiple seeds
                poison_meta_path = self.outdir / f"poisoned_samples_meta_seed_{seed}_pr_{poison_ratio}.pt"
                poison_meta_to_save = {
                    "seed": seed,
                    "target_class": self.config.target_class,
                    "non_target_class": self.config.non_target_class,
                    "poison_ratio": poison_ratio,
                    # training poison (protocol A): target subset indices
                    "poison_indices_in_target": poison_meta.get("poison_indices_in_target"),
                    # inference/analysis: non-target+trigger indices (not used in training)
                    "poison_indices_in_non_target": poison_meta.get("poison_indices_in_non_target"),
                    "target_indices": poison_meta.get("target_indices"),
                    "non_target_indices": poison_meta.get("non_target_indices"),
                    # pre-processed tensors for downstream per-layer feature / separability analysis
                    "target_clean_data": poison_meta.get("target_clean_data"),
                    "poisoned_target_data": poison_meta.get("poisoned_target_data"),
                    "non_target_clean_data": poison_meta.get("non_target_clean_data"),
                    "poisoned_non_target_data": poison_meta.get("poisoned_non_target_data"),
                    "trigger_tensor": poison_meta.get("trigger_tensor"),
                    "n_poison_target": poison_meta.get("n_poison_target"),
                    "n_trigger_eval": poison_meta.get("n_trigger_eval"),
                }
                torch.save(poison_meta_to_save, poison_meta_path)
                log.info(f"💾 Poisoned samples meta saved to {poison_meta_path}")
            except Exception as e:
                log.warning(f"⚠️ Failed to save poisoned samples meta for pr={poison_ratio}: {e}")
        ...
        seed_results['validation'] = {
            ...
            'clean_sep_epoch': clean_sep_epochs
        }
        
        return seed_results
    
    def _train_clean_baseline(self, processed_splits: Dict):
        """Train clean baseline QNN; log every epoch; compute separability at epochs multiples of 50"""
        ...
        opt = torch.optim.Adam(model.parameters(), lr=self.config.qnn_lr)
        criterion = nn.CrossEntropyLoss()

        # store separability at specific epochs
        clean_sep_epochs = {}  # e.g. {50: {...}, 100: {...}, ...}
        ...
        for epoch in range(self.config.qnn_epochs):
            ...
            for x, y in dl:
                x = self.device_manager.safe_to(x, self.device)
                y = self.device_manager.safe_to(y, self.device)
            ...
            # === record separability at every 50th epoch ===
            epoch_idx = epoch + 1
            if epoch_idx % 50 == 0 or epoch_idx == self.config.qnn_epochs:
                try:
                    analyzer = QuantumFeatureAnalyzer(model, self.config, self.device_str)
                    sep = analyzer.validate_class_separability(processed_splits, split='train', n_samples_per_class=200)
                    clean_sep_epochs[epoch_idx] = sep
                    log.info(f"🔎 Separability @ epoch {epoch_idx}: ratio={sep['separability_ratio']:.4f} "
                            f"(between={sep['between_class_distance']:.4f}, within={sep['average_within_class_variance']:.4e})")
                    # Avoid overwriting across multiple seeds: include actual seed + epoch
                    save_path = self.outdir / "models" / f"clean_model_seed_{self.config.random_seed}_epoch_{epoch_idx}.pt"
                    torch.save(model.state_dict(), save_path)
                    log.info(f"💾 Clean model saved to {save_path}")
                except Exception as e:
                    log.warning(f"Separability calc failed at epoch {epoch_idx}: {e}")

        log.info("✅ Clean baseline model training complete")
        return model, clean_sep_epochs
    ...
    def _create_poisoned_dataset(self, train_data: Dict, trigger: torch.Tensor, 
                                 poison_ratio: float) -> Tuple[DataLoader, Dict]:
        """
        Create clean-label poisoned dataset with robust device management.

        Black-box clean-label backdoor (protocol A):
        - Training poison: randomly sample target-class subset, add trigger, keep label as target (=1).
        - Inference attack (ASR eval): add trigger to non-target, expect misclassification to target (=1).

        For compatibility with downstream per-sample/layer analysis scripts, also constructs
        non-target+trigger samples (not used in training, only for analysis/detection) and returns them in meta.

        Returns:
        - loader: for training the backdoored model (clean target + poisoned target + clean non-target)
        - poison_meta: for downstream analysis scripts only, contains:
          * poison_indices_in_target (target indices used for training poison)
          * poison_indices_in_non_target (indices for triggered non-target+trigger samples, for analysis)
          * target_clean_data / poisoned_target_data / non_target_clean_data / poisoned_non_target_data (pre-processed tensors)
        """
        log.info(f"🧪 Creating poisoned dataset (ratio: {poison_ratio})")
        
        # train_data is already pre-processed (standardised + PCA applied)
        ...
        trigger = self.device_manager.safe_to(trigger, device).type_as(target_data)
        
        n_target = target_data.shape[0]
        n_non_target = non_target_data.shape[0]

        # number of training poison samples: target-class count x poison ratio
        n_poison_target = int(n_target * poison_ratio)
        n_poison_target = min(n_poison_target, n_target)

        # explicit generator for reproducibility under same seed (independent of global RNG call order)
        g_target = torch.Generator().manual_seed(int(self.config.random_seed) + 10007)
        g_non_target = torch.Generator().manual_seed(int(self.config.random_seed) + 20009)

        if n_poison_target > 0:
            perm_t = torch.randperm(n_target, generator=g_target, device=device)
            poison_indices_in_target = perm_t[:n_poison_target]
            clean_target_indices = perm_t[n_poison_target:]

            poisoned_target = target_data[poison_indices_in_target] + trigger
            if self.config.standardize_features:
                poisoned_target = poisoned_target.clamp_(-3, 3)
            target_clean = target_data[clean_target_indices]
        else:
            poison_indices_in_target = torch.empty(0, dtype=torch.long, device=device)
            clean_target_indices = torch.arange(n_target, device=device)
            poisoned_target = target_data.new_empty((0,) + target_data.shape[1:])
            target_clean = target_data

        # non-target training data remains clean (label 0)
        non_target_clean = non_target_data

        # construct non-target + trigger samples (not for training; analysis/detection only, consistent with ASR protocol)
        n_trigger_eval = min(int(n_target * poison_ratio), n_non_target)
        if n_trigger_eval > 0:
            perm_nt = torch.randperm(n_non_target, generator=g_non_target, device=device)
            poison_indices_in_non_target = perm_nt[:n_trigger_eval]
            poisoned_non_target = non_target_data[poison_indices_in_non_target] + trigger
            if self.config.standardize_features:
                poisoned_non_target = poisoned_non_target.clamp_(-3, 3)
        else:
            poison_indices_in_non_target = torch.empty(0, dtype=torch.long, device=device)
            poisoned_non_target = non_target_data.new_empty((0,) + non_target_data.shape[1:])

        # assemble binary classification training data from (target_clean, poisoned_target, non_target_clean):
        #   class 1: target_clean ∪ poisoned_target  (clean-label: label kept as 1)
        #   class 0: non_target_clean
        all_target_train = torch.cat([target_clean, poisoned_target], dim=0)
        all_train_data = torch.cat([all_target_train, non_target_clean], dim=0)
        all_train_labels = torch.cat([
            torch.ones(all_target_train.shape[0], device=device),            # Target class = 1 (clean + poisoned target)
            torch.zeros(non_target_clean.shape[0], device=device),           # Clean non-target = 0
        ], dim=0).long()
        ...
        poison_meta = {
            # training poison: target subset indices
            "poison_indices_in_target": poison_indices_in_target.detach().cpu(),
            # inference/analysis: non-target+trigger indices (not used in training)
            "poison_indices_in_non_target": poison_indices_in_non_target.detach().cpu(),
            "target_indices": torch.arange(n_target, dtype=torch.long).cpu(),
            "non_target_indices": torch.arange(n_non_target, dtype=torch.long).cpu(),
            # pre-processed tensors (CPU), for direct use in downstream scripts
            "target_clean_data": target_clean.detach().cpu(),
            "poisoned_target_data": poisoned_target.detach().cpu(),
            "non_target_clean_data": non_target_clean.detach().cpu(),
            "poisoned_non_target_data": poisoned_non_target.detach().cpu(),
            "trigger_tensor": trigger.detach().cpu(),
            "n_poison_target": int(n_poison_target),
            "n_trigger_eval": int(n_trigger_eval),
        }

        return loader, poison_meta
    ...
    def _train_backdoored_model(self, train_loader: DataLoader, poison_ratio: float):
        ...
        # Avoid overwriting across multiple seeds/poison ratios
        save_path = self.outdir / "models" / f"backdoor_model_seed_{self.config.random_seed}_pr_{poison_ratio}.pt"
        torch.save(model.state_dict(), save_path)
        ...
        for poison_ratio in self.config.poison_ratios:
            if poison_ratio in aggregated_results:
                row = {
                    ...
                    'Escape_rate_mean': aggregated_results[poison_ratio].get('escape_rate', {'mean': 0})['mean'],
                    'Escape_rate_std': aggregated_results[poison_ratio].get('escape_rate', {'std': 0})['std'],
...
def run_parameter_sweep_experiment(base_config: EnhancedSCBConfig):
...
def main():
    """Enhanced main function with complete fixes"""
    parser = argparse.ArgumentParser(description="Enhanced SCB Quantum Backdoor Experiment (Complete Fixed Version)")
    
    parser.add_argument("--random-seed", type=int, default=42, 
                   help="Random seed for reproducibility (default: 42)")
    # Basic parameters
    parser.add_argument("--target-class", type=int, default=7)
    parser.add_argument("--non-target-class", type=int, default=3)
    parser.add_argument("--n-seeds", type=int, default=5)
    parser.add_argument("--poison-ratios", nargs="+", type=float, default=[0.1, 0.2, 0.4, 0.6])
    
    # Quantum parameters
    parser.add_argument("--n-qubits", type=int, default=5)  # FIXED: 2^5=32 matches PCA
    parser.add_argument("--n-layers", type=int, default=3)
    
    # Algorithm parameters
    parser.add_argument("--epsilon", type=float, default=0.8)  # FIXED: Increased for stronger triggers
    parser.add_argument("--fooling-threshold", type=float, default=0.6)
    
    # Experimental parameters
    parser.add_argument("--min-clean-accuracy", type=float, default=85.0)
    parser.add_argument("--fpr-threshold", type=float, default=0.01)
    
    # Preprocessing
    parser.add_argument("--apply-pca", action="store_true", help="Apply PCA dimensionality reduction")
    parser.add_argument("--pca-components", type=int, default=None)
    parser.add_argument("--standardize", action="store_true", help="Apply standardization")
    
    # Output
    parser.add_argument("--output-dir", type=str, default="results/enhanced_scb_experiment_fixed")
    parser.add_argument("--parameter-sweep", action="store_true", help="Run parameter sensitivity analysis")
    ...
    config.fooling_threshold = args.fooling_threshold
    config.min_clean_accuracy = args.min_clean_accuracy
    config.fpr_threshold = args.fpr_threshold
    config.apply_pca = args.apply_pca
    if args.pca_components is not None:
        config.pca_components = args.pca_components
    config.standardize_features = args.standardize
    config.output_dir = args.output_dir
    
    log.info("📋 Enhanced SCB Experiment Configuration (Complete Fixed Version):")
    log.info(f"  • Random seed: {config.random_seed}")
    log.info(f"  • Classes: {config.target_class} (target) vs {config.non_target_class} (non-target)")
    log.info(f"  • Data split: {config.train_ratio:.1f}:{config.val_ratio:.1f}:{config.test_ratio:.1f}")
    log.info(f"  • Seeds: {config.n_seeds}")
    log.info(f"  • Poison ratios: {config.poison_ratios}")
    log.info(f"  • Preprocessing: PCA={config.apply_pca}, Standardize={config.standardize_features}")
    log.info(f"  • Defense: FPR threshold={config.fpr_threshold}, Min CA={config.min_clean_accuracy}%")
    log.info(f"  • FIXED: LDA parameters w and b calculation in StatisticalDistanceCalculator")
    ...
    if args.parameter_sweep:
        sweep_results = run_parameter_sweep_experiment(config)
        log.info("✅ Parameter sweep complete")
